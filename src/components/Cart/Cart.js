...
const Cart = props=>{
  ...
  const submiOrderHandler = async(userData) =>{
    setIsSubmitting(true)
    await fetch(
      "https://react-http-a7947-default-rtdb.firebaseio.com/orders.json",
      {
        method: "POST",
        body: JSON.stringify({
          user: userData,
  ...
  const motalActions = <div className={classes.actions}>
  ...
  const cartModalContent = (
    <>
      ...
      {isCheckout && (
        <Checkout
          onConfirm={submiOrderHandler}
          onCancel={props.onClose}
        ></Checkout>
      )}
      {!isCheckout && motalActions}
    </>
  );
